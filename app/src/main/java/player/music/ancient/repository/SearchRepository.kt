...
class RealSearchRepository(
    ...
    suspend fun searchAll(context: Context, query: String?, filter: Filter): MutableList<Any> {
        ...
        query.let { searchString ->
            ...
            /** Radio Stations **/
            val radioStations: List<RadioStationEntity> = if (filter == Filter.NO_FILTER) {
                roomRepository.getAllRadioStationsSync().filter { station ->
                    station.name.lowercase().contains(searchString.lowercase())
                }
            } else {
                emptyList()
            }
            if (radioStations.isNotEmpty()) {
                results.add("Radio Stations")
                results.addAll(radioStations)
            }
        }
        return results
    }
