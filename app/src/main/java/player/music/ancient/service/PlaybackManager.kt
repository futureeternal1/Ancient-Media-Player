...
class PlaybackManager(val context: Context) {
    ...
    private var playbackCallbacks: Playback.PlaybackCallbacks? = null
    ...
    fun maybeSwitchToCrossFade(crossFadeDuration: Int): Boolean {
        /* Switch to AncientExoPlayer if CrossFade duration is 0 and
                Playback is not an instance of AncientExoPlayer */
        if (playback !is AncientExoPlayer && crossFadeDuration == 0) {
            if (playback != null) {
                playback?.release()
            }
            playback = null
            playback = AncientExoPlayer(context)
            return true
    ...
    private fun openAudioEffectSession() {
        audioEffectManager.onAudioSessionId(audioSessionId)
    ...
    private fun createLocalPlayback(): Playback {
        // Set AncientExoPlayer when crossfade duration is 0 i.e. off
        return if (PreferenceUtil.crossFadeDuration == 0) {
            AncientExoPlayer(context)
        } else {
            CrossFadePlayer(context)
        }
    }

    private fun ensurePlaybackForSong(song: Song) {
        val currentPlayback = playback
        val desiredPlayback = when {
            song.isRadio && currentPlayback !is AncientExoPlayer -> AncientExoPlayer(context)
            !song.isRadio &&
                currentPlayback is AncientExoPlayer &&
                PreferenceUtil.crossFadeDuration > 0 -> createLocalPlayback()
            !song.isRadio && currentPlayback is StreamMediaPlayerPlayback -> createLocalPlayback()
            else -> null
        } ?: return

        currentPlayback?.release()
        playback = desiredPlayback.apply {
