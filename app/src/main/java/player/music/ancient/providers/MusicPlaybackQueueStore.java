 ...
 */
package player.music.ancient.providers;
...
public class MusicPlaybackQueueStore extends SQLiteOpenHelper {
  ...
  private static final String IS_RADIO = "is_radio";
  ...
  private void createTable(@NonNull final SQLiteDatabase db, final String tableName) {
    ...
    builder.append(IS_RADIO);
    builder.append(" INT NOT NULL DEFAULT 0);");
    ...
    while (position < queue.size()) {
      ...
      try {
        for (int i = position; i < queue.size() && i < position + NUM_PROCESS; i++) {
          ...
          values.put("album_artist", song.getAlbumArtist());
          values.put(IS_RADIO, song.isRadio() ? 1 : 0);
